...
add_btn.addEventListener('click', () => {
    if (task_input.value === '') {
        showAlertMessage('Entre com a Task', 'error');
    } else {
        addToDo(task_input);
        saveToLocalStorage();
        showAllTodos();
        task_input.value = '';
        showAlertMessage('Task adicionada com sucesso', 'success');
    }
});
...
function deleteTodo(id) {
    ...
    saveToLocalStorage();
    showAlertMessage('Task deletada com sucesso', 'success');
    showAllTodos();
}
...
function editTodo(id) {
    ...
    add_btn.addEventListener('click', () => {
        ...
        showAlertMessage('Task atualizada com sucesso', 'success');
    });
}
...
function clearAllTodos() {
    if(todos.length > 0) {
        todos = [];
        saveToLocalStorage();
        showAlertMessage('Todas as tasks foram deletadas', 'success');
        showAllTodos();
    }else{
        showAlertMessage('Sem tasks para deletar', 'error');
    }
}